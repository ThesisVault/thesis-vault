...
describe("getUsersWithPaginationEndPoint", () => {
	describe("User is authorized", () => {
		it("should successfully return paginated users", async () => {
			...
			const result = await caller.getPaginatedUsers(request);
			...
			expect(userIds).toContain(seededUserWithPermission.id);
		});
		...
		it("should return an error if user does not have MANAGE_USER permission", async () => {
			...
			let errorMessage = "";
			try {
				await caller.getPaginatedUsers(request);
			} catch (error) {
